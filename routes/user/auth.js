...
router.post("/signup", (req, res) => {
    ...
    let bodyContent = "<!DOCTYPE html><html><body><p>Hi "+req.body.name+",</p><p>Your OTP is <b>"+req.body.otp+"<b></p></body></html>";
    let sendData = {
        sendTo: req.body.email,
                                ...
                                mailService.sendMailTo(sendData, function(err, response1) {
                                   ...
                                   if(!err && response1) {
                                    ...
                                    mailerStatus = 'Mail Sent Failed'
                                        ...
                                        let message =  urlencode("Your OTP is: "+req.body.otp+".\n\n"+req.body.smsUrl+" #"+req.body.otp)
                            
                                        let smsOptions = {
                                            method: 'get',                          
                                            url:'https://smsapi.24x7sms.com/api_2.0/SendSMS.aspx?APIKEY=JMF5PpDXSoe&MobileNo=91'+req.body.mobile+'&SenderID=dinpos&Message='+message+'&ServiceName=TEMPLATE_BASED&DLTTemplateID=1407160924265691874',                                                                               
                                        };
...
router.post("/resend_otp", function(req,res){
    req.body.otp = randomNumber();
    console.log("response",req.body, "new_otp",req.body.otp );
    userList.findOneAndUpdate({email: req.body.email},
    { $set: {otp: req.body.otp, otp_status: 'send'}}, function(err,response){
        if(!err && response){
         console.log("response-------------------",response);
         let message =  urlencode("Your OTP is: "+req.body.otp+".\n\n"+req.body.smsUrl+" #"+req.body.otp)
    ...
    { $set: {otp: req.body.otp, otp_status: 'send'}}, function(err,response){
        ...
        // };
           console.log(req.body)
            ...
            res.json({ status: true, message: "OTP send successfully.",otp: req.body.otp });
        }else{
            res.json({ status: false, error: err, message: "OTP sent failed." });
        }
    })
            ...
            if(req.body.type === 'sentotp')
            {
              req.body.otp = randomNumber();
                ...
                userList.findOneAndUpdate({_id:mongoose.Types.ObjectId(req.body.customer_id), company_id:req.body.company_id, 'branch.branch_id': req.body.branch_id},{$set: { otp: req.body.otp, otp_status: req.body.otp_status,OTPdatetime: req.body.OTPdatetime}}, function(err, response){
                   ...
                   // let message =  "Your OTP is: 12345./n@www.dinamic.io #12345";
                    let message =  urlencode("Your OTP is: "+req.body.otp+".\n\n"+req.body.smsUrl+" #"+req.body.otp)
                    if(!err && response){ 
                        let smsOptions = {
                            method: 'get',                          
                        ...
                        mailService.sendMailTo(sendData, function(err, response1) {
                            console.log("response email........", response1, "error........", err)
                                   ...
                                   if(req.body.smsType === 'production')
                                   {
                                      ...
                                      res.json({ status: true, customer_id: response._id, data:response, smsType:'Production', mailerStatus : mailerStatus, OTP : req.body.otp, err:err });
                                  }
                                  else
                                  {
                                     res.json({ status: true, customer_id: response._id, data:response,smsType:'development', mailerStatus : mailerStatus, OTP : req.body.otp  });
...
router.post("/validate_qr", function(req, res){
    ...
    valet.findOne({qrcode_link: req.body.qr, status: 'inactive', pos_branch_id:req.body.pos_branch_id}, function(err, response){
        if(!err && response){ 
...
router.post("/update_valet_status", function(req, res){
    ...
    {
        valet.findOneAndUpdate({ _id : req.body.valet_id},{$set: { status : 'active', valet_status : req.body.valet_status, pos_valet_id : req.body.pos_valet_id, current_user_details:req.body.user_details }},{ new : true }, function(err, response){
            if(!err && response){ 
                console.log('valet response.......', response)
                res.json({ status: true, data: response });
            }else{
                console.log('valet error.......', err)
                res.json({ status: false, data: "No records found" });
            }
        })
    }
...
router.post("/get_valet_details_STATUS", function(req, res){
    console.log("body.....", req.body); 
            valet.findOne({ _id:mongoose.Types.ObjectId(req.body.valet_id),pos_branch_id:req.body.pos_branch_id}, function(err, response){
            console.log("valet response1............",response)
            if(!err && response){ 
                res.json({ status: true, data: response });
                console.log("get true data.........")
...
router.post("/razorpay_webhook/:branch_id", function(req, res){
    ...
    paymentData.payment_details = {method:'orderData'};
    paymentTemp.create(paymentData, function(err, response) {
        console.log("response paymentTemp..........", response)
        console.log("err paymentTemp..........", err)
     });

    // const order_paid = (req, res, next) => {
    //     console.log("webhook1..............",req.body)
    //     let signature = req.headers["x-razorpay-signature"];
      
    //     let validated_signature = razorpay.validateWebhookSignature(JSON.stringify(req.body), signature, req.params.branch_id);
    //     console.log("webhook2..............",validated_signature);
    ...
    //     paymentJSON = req.body["payload"]["payment"]["entity"];
    //     console.log("webhook3..............",paymentJSON);
    //   };
    ...
    //     let validated_signature = razorpay.validateWebhookSignature(JSON.stringify(req.body), signature, req.params.branch_id);
    //     console.log("signature............................",validated_signature);
    ...
    // exports.razorpay_webhook = (req, res) => {
        // console.log("razorpay_webhook Data.............",req.body); 
