...
router.post("/check_mobile_social_login", (req, res) => {
    ...
    userList.findOne({email:email,mob_status:true}, function(err, response){
        if(!err && response){ 
            console.log("response mob ", response)
            res.json({ status: true, mobile: response.mobile });
...
router.post("/signup", (req, res) => {
    ...
    // let bodyContent = "<!DOCTYPE html><html><body><p>Hi "+req.body.name+",</p><p>Welcome to <b>DiNAMIC!</b></p><p>Get ready to have an experience worth remembering. That means instant access, unlimited service, advanced order customization, and access to quick service options.</p><p>This is your OTP. Use it to complete your registration.</p><p><b>"+req.body.otp+"<b></p></body></html>";
    // let sendData = {
    //     sendTo: req.body.email,
    ...
    emailexistence.check(req.body.email, function(err, response) {
        ...
        if(!err && response)
        {
            userList.findOne({ $or: [ { mobile: req.body.mobile }, { email: req.body.email } ] }, function(err, response) {
                if(!err && response) {
                    if(response.activation) {
                        ...
                        // UPDATE
                        userList.findOneAndUpdate({ $or: [ { mobile: req.body.mobile }, { email: req.body.email } ] },
                            ...
                            if(!err && response) {
                                ...
                                // mailService.sendMailTo(sendData, function(err, response) {
                                    // if(!err && response) {
                                        ...
                                        res.json({ status: true, customer_id: customerId });
                                    // }
                                    // else {
                                    //     res.json({ status: false, error: err, message: "Invalid email" });
                                    // }
                        ...
                        if(!err && response) {
                            ...
                            // mailService.sendMailTo(sendData, function(err, response) {
                                // if(!err && response) {
                                    let smsOptions = {
                                        method: 'get',                                      
                                        url: 'https://smsapi.24x7sms.com/api_2.0/SendSMS.aspx?APIKEY=f1HkUAPbVaK&MobileNo=91'+req.body.mobile+'&SenderID=DNAMIC&Message=Welcome to DiNAMIC. Your OTP is '+req.body.otp+'.&ServiceName=TEMPLATE_BASED',   
                                    };
...
router.post("/check_mobile_number_exist", function(req, res){
    ...
    userList.find({ mobile: mobile, activation: true}, function(err, response){
        if(!err && response){   
            if(response.length){
...
router.post("/email_confirmation", function(req, res) {
    ...
    userList.findOne({ _id: mongoose.Types.ObjectId(req.body.user) }, function(err, response) {
        if(!err && response) {
            ...
            let recoveryLink = "http://localhost:4200/#/confirm_user_email/"+userid;
...
router.post("/save_social_user", function(req, res) {   
    ...
    userList.findOne({email:req.body.email}, function(err, response){
        if(!err && response){ 
            ...
            userList.findOneAndUpdate({email:req.body.email},{$set: { otp: req.body.otp, otp_status: req.body.otp_status }}, function(err, response){
                if(!err && response){ 
                    let smsOptions = {
                        method: 'get',                                      
                        //  url: 'http://smsapi.24x7sms.com/api_2.0/SendSMS.aspx?APIKEY=YYPN9lBliwW&MobileNo=91'+req.body.mobile+'&SenderID=DNAMIC&Message=Welcome to DiNAMIC. Your OTP is '+new_otp+'.&ServiceName=TEMPLATE_BASED',   
...
router.post("/razor_order_api", function(req, res){
    ...
    var instance = new Razorpay({
        key_id: 'rzp_test_oftSLbkQktK8gf',
        key_secret: 'gIIRWEWw8mRadZj2A0Sdr4IX'
      })
...
router.post("/validate_qr", function(req, res){
    ...
    valet.findOneAndUpdate({qrcode_link: req.body.qr, status: 'inactive'},{$set: { status: 'active', current_user_details: req.body.userDetails}},{ new : true }, function(err, response){
        if(!err && response){ 
...
router.post("/cancel_valet", function(req, res){
    ...
    valet.findOneAndUpdate({ _id : req.body._id},{$set: { status: 'inactive', current_user_details: {}}},{ new : true }, function(err, response){
        if(!err && response){ 
            res.json({ status: true, data: response });
        }else{
            res.json({ status: false, data: "No records found" });
...
router.post("/update_valet_status", function(req, res){
    ...
    if(req.body.valet_status != 'delivered'){
        ...
        valet.findOneAndUpdate({ _id : req.body.valet_id},{$set: { valet_status : req.body.valet_status, current_user_details : {}, status : 'inactive' }},{ new : true }, function(err, response){
            if(!err && response){ 
                res.json({ status: true, data: response });
            }else{
                res.json({ status: false, data: "No records found" });
...
router.post("/get_valet_details", function(req, res){
    ...
    valet.findOne({ "current_user_details.user_id": req.body.user_id, status: 'active'}, function(err, response){
        if(!err && response){ 
            res.json({ status: true, data: response });
        }else{
            res.json({ status: false, data: "No records found" });
