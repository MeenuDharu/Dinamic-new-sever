...
router.post("/validate_qr", function(req, res){
    ...
    valet.findOneAndUpdate({qrcode_link: req.body.qr, status: 'inactive', branch_id:mongoose.Types.ObjectId(response.branch_id)},{$set: {current_user_details: req.body.userDetails}},{ new : true }, function(err, response){
        if(!err && response){ 
             console.log("response of validate qr......", response)
            res.json({ status: true, data: response });
...
router.post("/get_valet_details", function(req, res){
    ...
    table.findOne({access_code:req.body.access_code}, function(err, response)
    {
        ...
        valet.findOne({ branch_id:mongoose.Types.ObjectId(response.branch_id),"current_user_details.dinamic_user_id": req.body.dinamic_user_id, status: 'active' }, function(err, response){
...
router.post("/get_valet_details_STATUS", function(req, res){
    ...
    {
        console.log("valet response............",response)
        valet.findOne({ branch_id:mongoose.Types.ObjectId(response.branch_id),"current_user_details.dinamic_user_id": req.body.dinamic_user_id }, function(err, response){
            console.log("valet response1............",response)
            if(!err && response){ 
                res.json({ status: true, data: response });
                console.log("get true data.........")
