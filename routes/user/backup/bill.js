...
router.post("/proceedtopay", function(req, res) {
    ...
    const amount = parseInt(req.body.cart_total, 10) //1234
...
router.post("/getpaymentrequeststatus/:branch_id/:id", function(req, res) {
    ...
    console.log("params2........",req.params.branch_id);
        ...
        gateway.findOne({branch_id:mongoose.Types.ObjectId(req.params.branch_id)}, function(err, response)
    {
        if(!err && response){ 
        let instance = new Razorpay({
            "key_id" : response.key_id,
...
router.post("/payment_details", function(req, res){
    ...
    table.findOne({access_code:req.body.access_code}, function(err, response)
    {
        ...
        if(!err && response){ 
            req.body.branch_id = response.branch_id
